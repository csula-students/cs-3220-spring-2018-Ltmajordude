...
class Generator {
  ...
  */
	getCost() {
		var cost = this.baseCost * Math.pow(1 + _constants2.default.growthRatio, this.quantity);
...
function loop(store) {
	...
	store.state.generators.forEach(function (item) {
		const generator = new _generator2.default(Object.assign({}, item));
	...
	switch (action.type) {
		...
		case 'CHECK_STORY':
			...
			state.story.forEach(function (item) {
				...
				const storyItem = new _story2.default(Object.assign({}, item));
...
class Story {
  ...
  */
	isUnlockYet(value) {
		...
		if (value >= this.triggeredAt) return true;else return false;
	}
